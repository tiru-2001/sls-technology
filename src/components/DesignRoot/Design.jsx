...
const wrapTextInSpans = (element) => {
  ...
  element.innerHTML = text.split("").map(char =>
    char === " " ? "&nbsp;" : `<span class="letter" style="display:inline-block;opacity:0;">${char}</span>`
  ).join("");
};

const getSortedLetters = (text) => {
  return text.replace(/\s+/g, '').split('').sort((a, b) => a.localeCompare(b));
};
  ...
  useGSAP(() => {
    ...
    cardRefs.current.forEach(card => {
      ...
      sortedLetters.forEach((letter, index) => {
        const targetSpans = Array.from(h5.querySelectorAll(".letter")).filter(span => span.textContent === letter);
